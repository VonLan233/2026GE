...
- **桌面通知** — 选课成功时弹窗 + 声音提醒（支持 macOS / Windows / Linux）
- **断线重连** — 自动处理 SSL 断连和 Session 过期

## 项目结构

```
├── scraper.py          # 入口（向后兼容，等同 python -m xmum）
├── xmum/               # 核心包
│   ├── __init__.py
│   ├── __main__.py     # CLI 参数解析
│   ├── constants.py    # URL、颜色、日志工具
│   ├── session.py      # HTTP 会话（登录、重试、ViewState）
│   ├── parser.py       # HTML 解析（课程表、学分、分页）
│   ├── notify.py       # 跨平台桌面通知
│   └── commands.py     # 子命令实现（query / grab / dump）
...
```bash
# 默认每 5 秒一轮
python scraper.py grab

# 自定义间隔（秒）
python scraper.py grab --interval 3

# 急速模式：~0.3 秒一轮，适合选课刚开放或蹲退课
python scraper.py grab --rush
```
...
- `ENTRY_ID` 每个选课轮次不同，需在 `xmum/constants.py` 中更新（当前为 `1403`）
